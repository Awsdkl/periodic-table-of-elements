...
public class Battery_ICT_recipes extends ICT_Recipe
{
    ...
    public boolean matches(DefaultedList<ItemStack> inventory)
    {
        ...
        if(inventory.get(3).getItem() != PeriodicTableOfElements.TIN_PLATE) return false;
        if(inventory.get(4).getItem() != Items.REDSTONE) return false;
        if(inventory.get(5).getItem() != PeriodicTableOfElements.TIN_PLATE) return false;
        if(inventory.get(6).getItem() != PeriodicTableOfElements.TIN_PLATE) return false;
        if(inventory.get(7).getItem() != Items.REDSTONE) return false;
        return inventory.get(8).getItem() == PeriodicTableOfElements.TIN_PLATE;
    }

    @Override
    public ItemStack craft(DefaultedList<ItemStack> inventory)
    {
        ItemStack stack = new ItemStack(PeriodicTableOfElements.BATTERY);
